...
var USUARIOS_PADRAO_DF = [
  {
    ...
    nome: 'Administrador SEEDF',
    get senha() { return getSeedCredential_('admin.seedf@se.df.gov.br'); },
    perfil: 'ADMIN',
    orgao: 'SEEDF/DIAE/GPAE',
    ativo: true,
  ...
  },
  {
    ...
    get senha() { return getSeedCredential_('coord.crepp@se.df.gov.br'); },
    perfil: 'GESTOR',
    cre: 'CRE-PP',
  ...
  },
  {
    ...
    get senha() { return getSeedCredential_('nutricionista.crepp@se.df.gov.br'); },
  ...
  },
  {
    ...
    get senha() { return getSeedCredential_('diretor.ec308sul@se.df.gov.br'); },
  ...
  },
  {
    ...
    get senha() { return getSeedCredential_('analista.crepp@se.df.gov.br'); },
    perfil: 'ANALISTA',
    cre: 'CRE-PP',
    orgao: 'CRE Plano Piloto',
    ativo: true
  },
  {
    ...
    get senha() { return getSeedCredential_('consulta@se.df.gov.br'); },
      ...
      try {
        try {
          ...
          return {
            ...
            credenciais: {
              admin: { email: 'admin.seedf@se.df.gov.br', senha: getSeedCredential_('admin.seedf@se.df.gov.br') },
              analista: { email: 'analista.crepp@se.df.gov.br', senha: getSeedCredential_('analista.crepp@se.df.gov.br') }
            }
          };
        ...
        Logger.log("Erro em criarUsuariosPadraoDF: " + error.message);
...
function listarUsuariosDF() {
  try {
    try {
      ...
      Logger.log("Erro em listarUsuariosDF: " + error.message);
      throw error;
    }
  } catch (error) {
    Logger.log("Erro em listarUsuariosDF: " + error.message);
...
function exibirRelatorioUsuariosDF() {
  try {
    ...
    Logger.log("Erro em exibirRelatorioUsuariosDF: " + error.message);
    throw error;
  }
}
  ...
  try {
    try {
      try {
        ...
        Logger.log('❌ Erro ao resetar credencial: ' + e.message);
        return {
          sucesso: false,
          erro: e.message
        };
      ...
      Logger.log("Erro em resetarSenhaUsuario: " + error.message);
      throw error; // Re-lança para tratamento superior
    }
  } catch (error) {
    Logger.log("Erro em resetarSenhaUsuario: " + error.message);
...
function adicionarMenuUsuarios() {
  try {
    ...
    Logger.log("Erro em adicionarMenuUsuarios: " + error.message);
    throw error;
  }
}
...
function menuResetarSenha() {
  try {
    ...
    Logger.log("Erro em menuResetarSenha: " + error.message);
    throw error;
  }
}
...
function gerarDocumentoCredenciais() {
  try {
    ...
    Logger.log("Erro em gerarDocumentoCredenciais: " + error.message);
    throw error;
  }
}
...
function registrarSetupUsuariosDF() {
  try {
    ...
    Logger.log("Erro em registrarSetupUsuariosDF: " + error.message);
    throw error;
  }
}
...
function criarUsuariosPadraoDF_Simples() {
  ...
  if (resultado.sucesso) {
    ...
    Logger.log('[LGPD] Evento registrado; detalhes sensíveis omitidos.');
    Logger.log('   Credencial: configurada nas propriedades/planilha do projeto');
    Logger.log('');
    Logger.log('6️⃣ CONSULTA');
    Logger.log('[LGPD] Evento registrado; detalhes sensíveis omitidos.');
    Logger.log('   Credencial: configurada nas propriedades/planilha do projeto');
    Logger.log('');
    Logger.log('⚠️ IMPORTANTE: altere as credenciais após o primeiro acesso!');
  } else {
...
function testarLoginUsuarioPadrao() {
  try {
    ...
    Logger.log("Erro em testarLoginUsuarioPadrao: " + error.message);
    throw error;
  }
}
