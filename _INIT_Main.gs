...
function doGet(e) {
  ...
  try {
    ...
    if (typeof UnifiedLogger !== 'undefined') {
      UnifiedLogger.log('Nova requisição', { params: params });
    } else {
    ...
    if (page === 'login') {
      return _rotearPagina('login', params);
    }
    if (page === 'setup') {
      return _servirPaginaSetup();
    }
    ...
    var sessaoId = params.sessao;
    var sessaoValida = false;
    if (sessaoId && typeof UnifiedAuth !== 'undefined') {
      sessaoValida = !!UnifiedAuth.getSession(sessaoId);
    } else if (sessaoId && typeof verificarSessao === 'function') {
      sessaoValida = verificarSessao(sessaoId);
    }

    if (!tokenValido && !sessaoValida) {
      if (typeof UnifiedLogger !== 'undefined') {
        UnifiedLogger.warn('Sessão ausente ou inválida, servindo login');
      }
      return _rotearPagina('login', params);
    }

    if (!page || page === 'app' || page === 'index') {
      return _servirIndexPrincipal();
    }

    return _rotearPagina(page, params);
  } catch (erro) {
    if (typeof UnifiedLogger !== 'undefined') {
...
function _servirIndexPrincipal() {
  try {
    ...
    Logger.log("Erro em _servirIndexPrincipal: " + error.message);
    throw error;
  }
}
  ...
  try {
    try {
      ...
      return template.evaluate()
        .setTitle('Preferencial SGAE — Acesso')
        .setFaviconUrl('https://www.gstatic.com/images/branding/product/1x/apps_script_48dp.png')
    ...
    Logger.log("Erro em _servirPaginaLogin: " + error.message);
    throw error;
  }
}
      ...
      <script>
        function executarSetup() {
          try {
            ...
            Logger.log("Erro em executarSetup: " + error.message);
            throw error;
          }
        }
      </script>
      ...
      <script>
        function fazerLogout() {
          try {
            ...
            Logger.log("Erro em fazerLogout: " + error.message);
            throw error;
          }
        }
      </script>
...
function _servirPaginaPerfil(params) {
  try {
    ...
    Logger.log("Erro em _servirPaginaPerfil: " + error.message);
    throw error;
  }
}
...
function _servirPaginaAjuda(params) {
  try {
    ...
    Logger.log("Erro em _servirPaginaAjuda: " + error.message);
    throw error;
  }
}
...
function _servirPagina404() {
  try {
    ...
    Logger.log("Erro em _servirPagina404: " + error.message);
    throw error;
  }
}
...
function _servirPaginaErro(erro) {
  try {
    ...
    Logger.log("Erro em _servirPaginaErro: " + error.message);
    throw error;
  }
}
...
function _verificarEstruturaBanco() {
  try {
    const ss = getSS();
    if (!ss) return false;
    // Aceita tanto setup via _SETUP_Project (Notas_Fiscais) quanto via criarEstruturaBancoDados (USR_Usuarios)
    return ss.getSheetByName('Notas_Fiscais') !== null ||
           ss.getSheetByName('USR_Usuarios') !== null;
  } catch (erro) {
    return false;
  }
}
...
function _init_createAdminMenu() {
  try {
    ...
    Logger.log("Erro em _init_createAdminMenu: " + error.message);
    throw error;
  }
}
...
function abrirSistema() {
  try {
    ...
    Logger.log("Erro em abrirSistema: " + error.message);
    throw error;
  }
}
...
function verificarIntegridadeMenu() {
  try {
    ...
    Logger.log("Erro em verificarIntegridadeMenu: " + error.message);
    throw error;
  }
}
...
function abrirCadastroUsuario() {
  try {
    ...
    var html = HtmlService.createTemplateFromFile('UI_CadastroUsuario').evaluate()
      .setWidth(450)
      .setHeight(650)
...
function atualizarEstruturaMenu() {
  try {
    ...
    Logger.log("Erro em atualizarEstruturaMenu: " + error.message);
    throw error;
  }
}
...
function migrarDadosMenu() {
  try {
    ...
    Logger.log("Erro em migrarDadosMenu: " + error.message);
    throw error;
  }
}
...
function limparDadosTesteMenu() {
  try {
    ...
    Logger.log("Erro em limparDadosTesteMenu: " + error.message);
    throw error;
  }
}
